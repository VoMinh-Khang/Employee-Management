...
CREATE OR REPLACE VIEW UV_DEANPHONGBAN
AS
    SELECT *
    FROM DEAN DA
    where DA.PHONG IN(SELECT NV.PHG
...
CREATE OR REPLACE VIEW UV_PHONGBANPHONGBAN
AS
    SELECT *
    FROM PHONGBAN PB
    where PB.MAPB IN(SELECT NV.PHG
                        FROM NHANVIEN NV
                        WHERE USERNAME=SYS_CONTEXT('USERENV','SESSION_USER'));
                        
--Cap quyen tren bang
GRANT SELECT ON UV_DEANPHONGBAN TO TRUONGPHONG;
GRANT SELECT ON UV_NHANVIENPHONGBAN TO TRUONGPHONG;
GRANT SELECT ON UV_PHONGBANPHONGBAN TO TRUONGPHONG;
...
SELECT NV.MANV, NV.TENNV, NV.PHAI, NV.NGAYSINH, NV.DIACHI, NV.SODT, F_DECRYPT_NHANVIEN(NV.LUONG) luong, F_DECRYPT_NHANVIEN(NV.PHUCAP) phucap, NV.VAITRO, NV.MANQL, NV.PHG, PC.MADA, PC.THOIGIAN, NV.USERNAME
FROM nvquantri.NHANVIEN NV 
INNER JOIN nvquantri.PHANCONG PC ON NV.MANV = PC.MANV;
...
CREATE OR REPLACE VIEW CS_NHANVIEN1 AS
    SELECT NV.MANV, NV.TENNV, NV.PHAI, NV.NGAYSINH, NV.DIACHI, NV.SODT, F_DECRYPT_NHANVIEN(NV.LUONG) luong, F_DECRYPT_NHANVIEN(NV.PHUCAP) phucap, NV.VAITRO, NV.MANQL, NV.PHG
    FROM nvquantri.NHANVIEN NV;

--Cs#4: Taichinh
grant select on nvquantri.CS_NHANVIEN1 to TAICHINH;
select * from CS_NHANVIEN1;
